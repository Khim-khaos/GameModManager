...
- **Кнопка экспорта модов во вкладке "Моды"**
  - Экспорт списка включённых модов в JSON формат
  - Сохранение информации: ID мода, название, автор
  - Экспорт привязан к текущей выбранной игре
...
- **Перенос кнопки экспорта из вкладки "Браузер" во вкладку "Моды"**
...
- **Ошибка обработчика событий игры (`_on_game_status_callback`)**
  - Исправлена сигнатура метода для корректной обработки событий `game_started`/`game_stopped`
- **Экспорт модов теперь видит включённые моды**
  - Использует `mod_manager.get_enabled_mods()` вместо кэша браузера
